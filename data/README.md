...
| binary_truncated_min_max.parquet | A file containing six columns with None, fully and partially truncated max and min statistics and with the expected is_{min/max}_value_exact.  (see [note](is_{min/max}_value_exact-statistics-file-on-binary_truncated_min_max)).|
...
## is_{min/max}_value_exact statistics file on binary_truncated_min_max
